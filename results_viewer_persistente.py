...
CLOUDFLARED_LOG_DIR = "/home/julio/cloudflared_logs"
    ...
    <script>
        ...
        function updateCloudflareUrl() {
            fetch('/api/cloudflare_url')
                .then(r => r.json())
                .then(data => {
                    document.getElementById('cloudflareUrl').textContent = data.url;
                })
                .catch(err => {
                    console.error('Error:', err);
                });
...
if __name__ == '__main__':
    ...
    print(f"Logs: ~/results_viewer.log")
    print(f"Para detener: pkill -f results_viewer_persistente.py")
