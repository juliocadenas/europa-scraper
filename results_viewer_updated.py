# Copia y pega este bloque completo en el servidor01
# Este script actualiza el servidor de resultados para mostrar archivos de results y omitidos

cat > ~/results_viewer_updated.py << 'PYEOF'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
OMITIDOS_DIR = "/home/julio/europa/omitidos"
...
class ResultsHandler(SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        elif path == '/api/list_omitidos':
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps(self.list_omitidos()).encode('utf-8'))
        elif path == '/omitidos':
            self.send_response(200)
            self.send_header('Content-type', 'text/html; charset=utf-8')
            self.end_headers()
            self.wfile.write(self.get_omitidos_html().encode('utf-8'))
        else:
    ...
    def list_omitidos(self):
        if not os.path.exists(OMITIDOS_DIR):
            return {"files": [], "message": "La carpeta omitidos no existe aun."}
    ...
    def download_file(self, filename):
        # Intentar descargar de results o omitidos
        filepath = os.path.join(RESULTS_DIR, filename)
    ...
    def preview_file(self, filename, rows):
        # Intentar previsualizar de results o omitidos
        filepath = os.path.join(RESULTS_DIR, filename)
        if not os.path.exists(filepath):
            filepath = os.path.join(OMITIDOS_DIR, filename)
...
<body>
    ...
    <script>
        ...
        function loadFiles() {
            const endpoint = currentTab === 'results' ? '/api/list' : '/api/list_omitidos';
...
if __name__ == '__main__':
    ...
    print(f"Sirviendo archivos de: {OMITIDOS_DIR}")
    server.serve_forever()
PYEOF

chmod +x ~/results_viewer_updated.py
pkill -f results_viewer_simple.py
nohup ~/results_viewer_updated.py > ~/results_viewer.log 2>&1 &

echo "Esperando 5 segundos..."
sleep 5

echo "Verificando que el servidor está corriendo..."
ps aux | grep results_viewer_updated
...
echo "Obteniendo URL de cloudflared..."
URL=$(grep -oP 'https://[a-zA-Z0-9\-]+\.trycloudflare\.com' ~/cloudflared_logs/*.log 2>/dev/null | tail -1)
...
echo "  ${URL}/"
echo ""
echo "URL de omitidos:"
echo "  ${URL}/omitidos"
echo ""
echo "========================================="
echo ""
echo "Logs del servidor:"
echo "  tail -f ~/results_viewer.log"
echo ""
