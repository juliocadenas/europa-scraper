...
Primero detiene cualquier trabajo en curso y luego ejecuta la prueba
"""
...
class ScrapingCleanTest:
    ...
    async def stop_current_job(self):
        """Detiene cualquier trabajo de scraping en curso"""
        print("🛑 Deteniendo trabajos de scraping en curso...")
        try:
            response = requests.post(f"{self.server_url}/stop_scraping", timeout=10)
            if response.status_code == 200:
                result = response.json()
                print(f"✅ Trabajos detenidos: {result.get('message', 'Sin mensaje')}")
                return True
            elif response.status_code == 404:
                print("ℹ️ No hay trabajos en curso")
                return True
            else:
                print(f"⚠️ Respuesta inesperada: {response.status_code}")
                return False
        except Exception as e:
            print(f"⚠️ Error deteniendo trabajos: {str(e)}")
            return False
    ...
    def load_test_data(self, limit: int = 3) -> List[Dict[str, Any]]:
    ...
    async def monitor_task_execution(self, task_id: str, timeout: int = 180):
    ...
    def save_test_report(self):
        ...
        report_file = f"test_report_clean_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    async def run_complete_test(self):
        ...
        print("🚀 Iniciando prueba completa de scraping end-to-end (con limpieza)")
        print("=" * 60)
        
        self.test_results["start_time"] = datetime.now()
        
        try:
            # Paso 0: Detener trabajos en curso
            await self.stop_current_job()
            await asyncio.sleep(2)  # Esperar a que se detengan completamente
            ...
            if not await self.monitor_task_execution(task_id, timeout=180):  # 3 minutos max
                return False
...
async def main():
    """Función principal"""
    print("🧪 Test End-to-End de Europa Scraper (con limpieza)")
