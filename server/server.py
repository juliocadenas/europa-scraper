...
class EventLog:
    ...
    async def add(
        self, event_type: str, source: str, message: str, details: Dict = None
    ):
...
NUM_PROCESSES = (
    os.cpu_count() or 4
)  # Usar todos los núcleos disponibles, o 4 como fallback
...
def worker_process(
    ...
    setup_logger(
        logging.DEBUG, os.path.join(project_root, "logs", f"worker_{worker_id}.log")
    )
    ...
    while True:
        try:
            ...
            if work_item is None:  # Señal de finalización
                ...
                log_event_sync(
                    EventType.WORKER, "Señal de finalización recibida. Apagando."
                )
                break
            ...
            logger.info(
                f"Worker {worker_id}: Motor de búsqueda del trabajo: '{search_engine_param}'"
            )
            ...
            if not browser_initialized:
                ...
                log_event_sync(
                    EventType.WORKER, "Inicializando componentes para la primera tarea."
                )
                ...
                class WorkerServerState:
                    ...
                    def set_pending_captcha_challenge(self, challenge):
                        logger.info(
                            f"CAPTCHA challenge detectado en worker: {challenge}"
                        )
                        log_event_sync(
                            EventType.WARNING,
                            "CAPTCHA challenge detectado.",
                            {"challenge": challenge},
                        )
                    ...
                    try:
                        with open(proxy_file_path, "r", encoding="utf-8") as f:
                            ...
                            if proxies:
                                ...
                                logger.info(
                                    f"Cargados {len(proxies)} proxies desde {proxy_file_path}"
                                )
                    except Exception as e:
                ...
                if config_manager.get("use_proxies", False):
                    proxy_manager.set_rotation_enabled(
                        config_manager.get("proxy_rotation", True)
                    )
                    proxy_manager.set_timeout(
                        int(config_manager.get("proxy_timeout", 30))
                    )
                    logger.info(
                        f"Worker {worker_id}: Proxies habilitados. {len(proxy_manager.proxies)} proxies cargados."
                    )
                    log_event_sync(
                        EventType.SYSTEM,
                        f"Proxies habilitados. {len(proxy_manager.proxies)} proxies cargados.",
                    )
                else:
                ...
                browser_manager = BrowserManager(
                    config_manager=config_manager, server_state=worker_server_state
                )
                ...
                try:
                    ...
                    if platform.system() == "Linux":
                        ...
                        if not is_headless_param:
                            logger.warning(
                                f"Worker {worker_id}: Forzando headless=True en Linux (solicitado: {is_headless_param})"
                            )
                            log_event_sync(
                                EventType.WARNING,
                                "Forzando headless=True en Linux.",
                                {"requested_headless": is_headless_param},
                            )
                    else:
                    ...
                    if search_engine_param == "Cordis Europa API":
                        logger.info(
                            f"Worker {worker_id}: Modo API detectado ({search_engine_param}). Omitiendo inicialización de navegador."
                        )
                        log_event_sync(
                            EventType.SYSTEM,
                            "Modo API detectado. Omitiendo inicialización de navegador.",
                        )
                    else:
                        logger.info(
                            f"Worker {worker_id}: Modo headless = {is_headless}"
                        )
                        log_event_sync(
                            EventType.SYSTEM, f"Modo headless = {is_headless}."
                        )
                        # Añadir un retraso aleatorio para evitar race conditions al iniciar múltiples navegadores
                        delay = random.uniform(2.0, 8.0)
                        logger.info(
                            f"Worker {worker_id}: Esperando {delay:.2f}s antes de inicializar navegador..."
                        )
                        time.sleep(delay)

                        loop.run_until_complete(
                            browser_manager.initialize(headless=is_headless)
                        )
                        logger.info(
                            f"Worker {worker_id}: Navegador inicializado correctamente"
                        )
                        log_event_sync(
                            EventType.SYSTEM, "Navegador inicializado correctamente."
                        )
                except Exception as e:
                    import traceback
                ...
                # INYECTAR SEMÁFORO GLOBAL AL CLIENTE DE API
            ...
            if not scraper_controller or not browser_manager:
                logger.error(
                    f"Worker {worker_id}: Controlador o BrowserManager no inicializados, saltando lote."
                )
                log_event_sync(
                    EventType.ERROR,
                    "Controlador o BrowserManager no inicializados, saltando lote.",
                )
                if "work_queue" in locals():
                    work_queue.task_done()
                continue
            ...
            course_name_display = (
                batch[0][1] if batch and len(batch) > 0 else "Desconocido"
            )

            logger.info(
                f"Worker {worker_id} recibió lote {batch_id} con {len(batch)} cursos y parámetros: {job_params}"
            )
            log_event_sync(
                EventType.SCRAPER,
                f"Recibido lote {batch_id} para {len(batch)} cursos.",
            ...
            log_event_sync(
                EventType.ERROR, error_msg, {"traceback": traceback.format_exc()}
            )
            status_dict[worker_id] = {
                "id": worker_id,
                "status": "Error",
...
class ScraperServer:
    ...
    async def _startup(self):
        ...
        await global_event_log.add(
            EventType.SYSTEM,
            "Server",
            "Iniciando servidor y gestor de multiprocesamiento.",
        )
        ...
        # Semáforo nativo del SO: máximo 3 peticiones simultáneas a CORDIS
        self.api_semaphore = multiprocessing.Semaphore(3)
        ...
        await global_event_log.add(
            EventType.SYSTEM,
            "Server",
            "Iniciado el monitor de limpieza de workers y consumidor de eventos.",
        )
    ...
    async def _shutdown(self):
        ...
        if hasattr(self, "event_consumer_stop"):
            ...
            if self.event_consumer_thread.is_alive():
                self.logger.warning(
                    "Event consumer thread did not terminate gracefully."
                )

        if self.manager:
            self.logger.info("Cerrando el gestor de multiprocesamiento...")
            await global_event_log.add(
                EventType.SYSTEM, "Server", "Cerrando el gestor de multiprocesamiento."
            )
    ...
    def _start_worker_pool(self, num_workers: int):
        if self.worker_pool:
            self.logger.warning(
                "El pool de trabajadores ya está en ejecución. No se iniciará de nuevo."
            )
            asyncio.create_task(
                global_event_log.add(
                    EventType.WARNING,
                    "Server",
                    "Intento de iniciar pool de workers, pero ya está en ejecución.",
                )
            )
            return
        ...
        asyncio.create_task(
            global_event_log.add(
                EventType.SYSTEM,
                "Server",
                f"Iniciando un pool de {num_workers} procesos trabajadores.",
            )
        )
        self.worker_pool = [
        ...
        try:
            ...
            loop.create_task(
                global_event_log.add(
                    EventType.SYSTEM,
                    "Server",
                    "Enviando señal de finalización a todos los trabajadores.",
                )
            )
        except RuntimeError:
            asyncio.run(
    ...
    def _start_broadcasting(self):
        ...
        self.broadcast_thread = threading.Thread(
            target=self._broadcast_server_presence, daemon=True
        )
        ...
        asyncio.run(
            global_event_log.add(
                EventType.SYSTEM,
                "Server",
                "Iniciado broadcasting de presencia del servidor.",
            )
        )
    ...
    def _stop_broadcasting(self):
        ...
        if hasattr(self, "broadcast_thread") and self.broadcast_thread.is_alive():
            ...
            asyncio.run(
                global_event_log.add(
                    EventType.SYSTEM, "Server", "Deteniendo broadcasting del servidor."
                )
            )
        ...
        while True:
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
                    ...
                    sock.sendto(
                        message.encode("utf-8"), ("<broadcast>", BROADCAST_PORT)
                    )
                ...
                asyncio.run(
                    global_event_log.add(
                        EventType.ERROR, "Server", f"Error enviando broadcast: {e}"
                    )
                )
    ...
    def _setup_routes(self):
        ...
        self.app.get("/api/results_line_count")(self.get_results_line_count)
        self.app.get("/api/download_file")(self.download_single_file)
        self.app.delete("/api/delete_file")(
            self.delete_single_file
        )  # NUEVO: borrado individual
        ...
        for route in self.app.routes:
            ...
            asyncio.run(
                global_event_log.add(
                    EventType.SYSTEM,
                    "Server",
                    f"Ruta registrada: {route.methods} {route.path}",
                )
            )
    ...
    async def start_scraping_job(self, request_data: dict):
        self.logger.info(
            f"¡¡¡ENDPOINT /start_scraping ALCANZADO!!! Datos: {request_data}"
        )
        await global_event_log.add(
            EventType.SYSTEM,
            "Server",
        ...
        if self.worker_states is not None:
            ...
            await global_event_log.add(
                EventType.SYSTEM,
                "Server",
                "Limpiando estados previos de los trabajadores.",
            )
        ...
        try:
            ...
            if "job_params" in request_data:
                ...
                self.logger.info(
                    f"Usando formato anidado: {from_sic} a {to_sic}, workers={num_workers}, mode={search_mode}"
                )
            else:
            ...
            await global_event_log.add(
                EventType.SYSTEM,
                "Server",
                f"Parámetros de trabajo procesados: {from_sic} a {to_sic}, {num_workers} workers.",
                {"job_params": job_params},
            )
            ...
            if not os.path.exists(db_path):
                await global_event_log.add(
                    EventType.ERROR,
                    "Server",
                    "La base de datos 'courses.db' no existe. Por favor, suba un archivo de cursos primero.",
                )
                raise HTTPException(
                    status_code=404,
                    detail="La base de datos 'courses.db' no existe. Por favor, suba un archivo de cursos primero.",
                )
            db_handler = SQLiteHandler(db_path)
            all_courses = (
                db_handler.get_all_courses()
            )  # Devuelve lista de tuplas (sic_code, course_name)

            # 2. Filtrar cursos basados en el rango SIC del trabajo
            start_index = next(
                (i for i, (sic, _) in enumerate(all_courses) if sic == from_sic), 0
            )
            end_index = next(
                (i for i, (sic, _) in enumerate(all_courses) if sic == to_sic),
                len(all_courses) - 1,
            )
            ...
            if not courses_to_process:
                ...
                await global_event_log.add(
                    EventType.WARNING,
                    "Server",
                    "No se encontraron cursos en el rango SIC especificado.",
                )
                raise HTTPException(
                    status_code=404,
                    detail="No se encontraron cursos en el rango SIC especificado.",
                )

            # 3. Crear diccionario de parámetros para los workers
            # Incluir results_output_mode del request
            results_output_mode = (
                request_data.get("results_output_mode", "Por curso")
                if isinstance(request_data, dict)
                else "Por curso"
            )
            if "job_params" in request_data and isinstance(
                request_data["job_params"], dict
            ):
                results_output_mode = request_data["job_params"].get(
            ...
            self.logger.info(
                f"Encolando {num_courses} cursos como tareas individuales."
            )
            ...
            self.logger.info(
                f"¡¡¡TAREAS PUESTAS EN LA COLA!!! Tamaño de la cola: {self.work_queue.qsize()}"
            )
            await global_event_log.add(
                EventType.SYSTEM,
                "Server",
            ...
            if self.worker_pool and len(self.worker_pool) != num_workers:
                self.logger.info(
                    f"Reiniciando pool: Solicitados {num_workers}, Activos {len(self.worker_pool)}"
                )
                await global_event_log.add(
                    EventType.SYSTEM,
                    "Server",
            ...
            monitor_thread = threading.Thread(
                target=self._monitor_job_completion, daemon=True
            )
            ...
            try:
                await global_event_log.add(
                    EventType.ERROR,
                    "Server",
                    f"Error al iniciar el trabajo de scraping: {e}",
                    {"traceback": traceback.format_exc()},
                )
            ...
            self.is_job_running = False
            raise HTTPException(
                status_code=500, detail=f"Error al iniciar el trabajo: {e}"
            )
    ...
    async def stop_scraping_job(self):
        if not self.is_job_running:
            await global_event_log.add(
                EventType.WARNING,
                "Server",
                "Intento de detener scraping, pero no hay ningún trabajo en ejecución.",
            )
            raise HTTPException(
                status_code=404,
                detail="No hay ningún trabajo de scraping en ejecución.",
            )

        self.logger.info(
            "Se recibió una solicitud para detener el trabajo de scraping."
        )
        await global_event_log.add(
            EventType.SYSTEM,
            "Server",
        ...
        return {
            "message": "Solicitud de detención recibida. El pool de trabajadores ha sido detenido."
        }
    ...
    async def force_reset_state(self):
        ...
        await global_event_log.add(
            EventType.WARNING, "Server", "SOLICITUD DE REINICIO FORZADO RECIBIDA."
        )
        ...
        await global_event_log.add(
            EventType.SUCCESS,
            "Server",
            "Servidor reiniciado forzosamente. El estado ahora es 'Inactivo'.",
        )
        return {
            "message": "Servidor reiniciado forzosamente. El estado ahora es 'Inactivo'."
        }
    ...
    async def get_all_courses(self):
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error interno del servidor al obtener cursos: {e}",
            )

    async def upload_courses(self, file: UploadFile = File(...)):
        self.logger.info(
            f"Recibida solicitud para cargar archivo de cursos: {file.filename}"
        )

        if not file.filename.endswith((".csv", ".xlsx")):
            raise HTTPException(
                status_code=400,
                detail="Formato de archivo no soportado. Por favor, suba un archivo .csv o .xlsx.",
            )
        ...
        try:
            ...
            if len(df.columns) < 2:
                raise HTTPException(
                    status_code=400,
                    detail="El archivo requiere al menos 2 columnas (CÃ³digo y Nombre).",
                )
            ...
            self.logger.info(
                f"Usando columna 0 ('{code_col_orig}') como CÃ³digo y columna 1 ('{course_col_orig}') como Curso."
            )
            ...
            courses_to_load = [
                c for c in courses_to_load if c[0] and c[1] and c[0].lower() != "nan"
            ]

            self.logger.info(
                f"Se encontraron {len(courses_to_load)} cursos en el archivo."
            )
            ...
            if success:
                ...
                self.logger.info(
                    f"VERIFICACIÓN: Leídos {verify_count} cursos de la DB inmediatamente después de insertar."
                )

                if verify_count == 0:
                    self.logger.error(
                        "ALERTA CRÍTICA: La base de datos está vacía después de una inserción supuestamente exitosa."
                    )
                    raise HTTPException(
                        status_code=500,
                        detail="Error Crítico: Los cursos se procesaron pero no se guardaron en la base de datos (0 filas encontradas).",
                    )

                return {
                    "message": f"Carga exitosa. Se han cargado {len(courses_to_load)} cursos en la base de datos. Verificación: {verify_count} registros."
                }
            else:
                raise HTTPException(
                    status_code=500,
                    detail="Error interno: No se pudieron guardar los cursos en la base de datos (SQLite error).",
                )
    ...
    async def list_results_files(self):
        ...
        try:
            ...
            for root, dirs, files in os.walk(results_dir):
                for filename in files:
                    if filename.endswith(".csv") or filename.endswith(".xlsx"):
                        ...
                        category = "General"
                        if "/EN/" in f"/{relative_path}/":
                            category = "EN"
                        elif "/ES/" in f"/{relative_path}/":
                            category = "ES"
                        elif "/omitidos/" in f"/{relative_path}/":
                            category = "Omitidos"
            ...
            raise HTTPException(
                status_code=500, detail=f"Error listando archivos: {str(e)}"
            )

    async def get_results_line_count(self):
        """Cuenta el total de líneas de todos los archivos CSV en results."""
        try:
            results_dir = os.path.join(project_root, "results")
            self.logger.info(f"Contando líneas en: {results_dir}")

            if not os.path.exists(results_dir):
                return {
                    "total_lines": 0,
                    "total_files": 0,
                    "message": "Carpeta results no existe",
                }
            ...
            self.logger.info(f"Total: {total_lines} líneas en {total_files} archivos")
            return {
                "total_lines": total_lines,
            ...
            raise HTTPException(
                status_code=500, detail=f"Error contando líneas: {str(e)}"
            )
    ...
    async def list_omitidos_files(self):
        ...
        try:
            ...
            for omitidos_dir in dirs_to_check:
                if os.path.exists(omitidos_dir):
                    for filename in os.listdir(omitidos_dir):
                        if (
                            filename.endswith(".csv") or filename.endswith(".xlsx")
            ...
            raise HTTPException(
                status_code=500, detail=f"Error listando archivos omitidos: {str(e)}"
            )
    ...
    async def download_single_file(self, filename: str):
        ...
        try:
            ...
            raise HTTPException(
                status_code=404, detail=f"Archivo no encontrado: {filename}"
            )
        ...
        try:
            ...
            for directory in search_paths:
                ...
                if os.path.exists(filepath) and os.path.isfile(filepath):
                    ...
                    await global_event_log.add(
                        EventType.SYSTEM, "Server", f"Archivo eliminado: {safe_name}"
                    )
                    break

            if not deleted:
                raise HTTPException(
                    status_code=404, detail=f"Archivo no encontrado: {safe_name}"
                )
...
<body>
    ...
    async def get_cloudflare_url(self):
        ...
        try:
            ...
            log_files = sorted(
                [
                    f
                    for f in os.listdir(cloudflare_log_dir)
                    if f.startswith("cloudflared_")
                ],
            ...
            return {
                "url": None,
                "message": "No se encontró URL en los logs de cloudflared",
            }
    ...
    async def emergency_reset(self):
        ...
        await global_event_log.add(
            EventType.SYSTEM, "Server", "Iniciando RESET DE EMERGENCIA."
        )
    ...
    async def debug_info(self):
        ...
        try:
            return {
                ...
                "files_in_results": len(
                    os.listdir(os.path.join(project_root, "results"))
                )
                ...
                "environment": {
                    ...
                    if "KEY" not in k and "AUTH" not in k
                },
            ...
            if self.worker_states is not None:
                ...
                for worker_id, worker_data in workers_dict.items():
                    ...
                    if (
                        ...
                        or "CRASHED" in str(current_task)
                        or "Error" in str(current_task)
                    ):
    ...
    async def get_course_details(self, sic_code: str = Query(...)):
        ...
        try:
            ...
            return {
                ...
                "worker_log_entries": worker_logs[-20:]
                if worker_logs
                else [],  # Últimas 20 entradas
            }

        except Exception as e:
            self.logger.error(f"Error obteniendo detalles del curso {sic_code}: {e}")
            raise HTTPException(
                status_code=500, detail=f"Error obteniendo detalles: {str(e)}"
            )
