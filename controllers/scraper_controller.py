...
class ScraperController(ScraperControllerBase):
      ...
      try:
          async with self._processing_semaphore:
              ...
              if record_identifier in self.processed_records:
                  ...
                  self._emit_event("WARNING", f"Registro duplicado omitido: {url}", {"sic": sic_code, "course": course_name})
                  return None
              ...
              if is_cordis and description and len(description) > 50:
                  ...
                  try:
                      ...
                      self._emit_event("WARNING", f"Omitido por timeout en extracción de contenido: {url}", {"sic": sic_code, "course": course_name})
                      return None
              ...
              if not full_content:
                  ...
                  self._emit_event("WARNING", f"Omitido por error en extracción de contenido: {url}", {"sic": sic_code, "course": course_name})
                  return None
              ...
              if should_exclude:
                  ...
                  if 'total keywords' in exclude_reason.lower():
                      ...
                      self._emit_event("WARNING", f"Omitido por bajo contenido ({total_keywords} palabras clave)", {"url": url, "min_words": min_words})
                      ...
                      self._emit_event("WARNING", f"Omitido por cero palabras clave: {url}", {"sic": sic_code, "course": course_name})
                  return None
              ...
              if formatted_word_counts.startswith('Total words:') and len(formatted_word_counts.split('|')) == 1:
                  ...
                  self._emit_event("WARNING", f"Omitido por cero palabras clave (formato): {url}", {"sic": sic_code, "course": course_name})
                  return None
          ...
          self._emit_event("ERROR", f"Error procesando URL: {e}", {"url": result.get('url','')})
          return None
  ...
  def _get_courses_in_range_by_position(self, all_courses: List[Tuple[str, str, str, str]], from_sic: str, to_sic: str) -> List[Tuple[str, str, str, str]]:
      ...
      try:
          ...
          if from_index is None:
              ...
              self._emit_event("ERROR", f"Código 'desde' no encontrado: {from_sic}", {"from_sic": from_sic})
              return []
          
          if to_index is None:
              logger.error(f"Código 'hasta' no encontrado: {to_sic}")
              self._emit_event("ERROR", f"Código 'hasta' no encontrado: {to_sic}", {"to_sic": to_sic})
              return []
          ...
          self._emit_event("ERROR", f"Error calculando rango por posición: {str(e)}", {"from_sic": from_sic, "to_sic": to_sic})
          return []
  ...
  async def _process_search_phase(self, courses_in_range: List[Tuple[str, str, str, str]], progress_callback: Optional[Callable] = None, search_engine: str = 'DuckDuckGo', site_domain: Optional[str] = None) -> List[Dict[str, Any]]:
      ...
      for sic_code, course_name, status, server in courses_in_range:
          if self.stop_requested:
              ...
              self._emit_event("INFO", "Scraping detenido durante la fase de búsqueda")
              break
          ...
          self._emit_event("SEARCH", f"Iniciando búsqueda para '{search_term}'", {"sic": sic_code, "course": course_name, "engine": search_engine})
          ...
          try:
              if self.stop_requested:
                  ...
                  self._emit_event("INFO", "Scraping detenido antes de iniciar búsqueda")
                  break
              ...
              self._emit_event("SEARCH_COMPLETED", f"Búsqueda completada: {len(search_results)} resultados.", {"term": search_term, "engine": search_engine})
              ...
              self.stats['total_errors'] += 1
              self._emit_event("ERROR", f"Timeout durante la búsqueda de '{search_term}'", {"term": search_term})
              continue
          except Exception as e:
              logger.error(f"Error durante la búsqueda de '{search_term}': {str(e)}")
              logger.error(traceback.format_exc())
              self.stats['total_errors'] += 1
              self._emit_event("ERROR", f"Error durante la búsqueda de '{search_term}': {str(e)}", {"term": search_term})
              continue
  ...
  async def _process_common_crawl_search_phase(self, courses_in_range: List[Tuple[str, str, str, str]], site_domain: str, progress_callback: Optional[Callable] = None) -> List[Dict[str, Any]]:
      ...
      for sic_code, course_name, status, server in courses_in_range:
          if self.stop_requested:
              logger.info("Scraping detenido por el usuario durante la fase de búsqueda")
              self._emit_event("INFO", "Scraping detenido durante la fase de búsqueda (Common Crawl)")
              break
          ...
          self._emit_event("SEARCH", f"Iniciando búsqueda para '{search_term}' en Common Crawl", {"sic": sic_code, "course": course_name, "engine": "Common Crawl", "domain": site_domain})
          ...
          try:
              if self.stop_requested:
                  logger.info("Scraping detenido por el usuario antes de iniciar búsqueda")
                  self._emit_event("INFO", "Scraping detenido antes de iniciar búsqueda (Common Crawl)")
                  break
              ...
              if self.stop_requested:
                  logger.info("Scraping detenido por el usuario después de búsqueda")
                  self._emit_event("INFO", "Scraping detenido después de búsqueda (Common Crawl)")
                  break
              ...
              self.stats['total_urls_found'] += len(search_results)
              logger.info(f"Encontrados {len(search_results)} resultados para '{search_term}'")
              self._emit_event("SEARCH_COMPLETED", f"Búsqueda completada en Common Crawl: {len(search_results)} resultados.", {"term": search_term, "engine": "Common Crawl"})
              ...
              logger.error(traceback.format_exc())
              self.stats['total_errors'] += 1
              self._emit_event("ERROR", f"Error durante la búsqueda en Common Crawl de '{search_term}': {str(e)}", {"term": search_term})
              continue
  ...
  async def _process_wayback_machine_search_phase(self, courses_in_range: List[Tuple[str, str, str, str]], site_domain: str, progress_callback: Optional[Callable] = None, gov_only: bool = False) -> List[Dict[str, Any]]:
      ...
      for sic_code, course_name, status, server in courses_in_range:
          if self.stop_requested:
              logger.info("Scraping detenido por el usuario durante la fase de búsqueda")
              self._emit_event("INFO", "Scraping detenido durante la fase de búsqueda (Wayback Machine)")
              break
          ...
          self._emit_event("SEARCH", f"Iniciando búsqueda para '{search_term}' en Wayback Machine", {"sic": sic_code, "course": course_name, "engine": "Wayback Machine", "domain": site_domain})
          ...
          try:
              ...
              self._emit_event("SEARCH_COMPLETED", f"Búsqueda completada en Wayback Machine: {len(results)} resultados.", {"term": search_term, "engine": "Wayback Machine"})
              ...
              logger.error(traceback.format_exc())
              self.stats['total_errors'] += 1
              self._emit_event("ERROR", f"Error durante la búsqueda en Wayback para '{search_term}': {str(e)}", {"term": search_term})
              continue
  ...
  async def _process_cordis_api_phase(self, courses_in_range: List[Tuple[str, str, str, str]], search_mode: str = 'broad', progress_callback: Optional[Callable] = None) -> List[Dict[str, Any]]:
      ...
      for sic_code, course_name, status, server in courses_in_range:
          if self.stop_requested:
              logger.info("Scraping detenido por el usuario durante la fase de búsqueda")
              self._emit_event("INFO", "Scraping detenido durante la fase de búsqueda (Cordis API)")
              break
          ...
          try:
              ...
              self._emit_event("SEARCH_COMPLETED", f"Búsqueda completada en Cordis API: {len(results)} resultados.", {"term": search_term, "engine": "Cordis API"})
              ...
              self.stats['total_errors'] += 1
              self._emit_event("ERROR", f"Error en búsqueda Cordis API para '{search_term}': {e}", {"term": search_term})
              continue
  ...
  async def _process_tabulation_phase(self, all_search_results: List[Dict[str, Any]], total_courses: int, min_words: int, search_engine: str, progress_callback: Optional[Callable] = None, require_keywords: bool = False) -> List[Dict[str, Any]]:
      ...
      self._emit_event("INFO", f"Iniciando tabulación de {self.total_results_to_process} resultados encontrados")
      ...
      if self.stop_requested:
          ...
          self._emit_event("INFO", "Scraping detenido antes de iniciar la fase de tabulación")
      ...
      for i, ((sic_code, course_name), course_results) in enumerate(results_by_course.items()):
          if self.stop_requested:
              ...
              self._emit_event("INFO", "Scraping detenido durante la fase de tabulación")
              break
          ...
          for j in range(0, len(course_results), self._batch_size):
              if self.stop_requested:
                  ...
                  self._emit_event("INFO", "Scraping detenido durante el procesamiento de lotes")
                  break
              ...
              try:
                  ...
                  for task in done:
                      try:
                          ...
                          self._emit_event("ERROR", f"Error obteniendo resultado de tarea: {str(e)}")
      ...
      try:
          ...
          if search_engine not in ['Wayback Machine', 'Cordis Europa API']:
              ...
              if hasattr(self, 'content_extractor'):
                  ...
                  logger.info("Browser fallback DISABLED in ContentExtractor for API-only mode.")
