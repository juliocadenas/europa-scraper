...
class ClientApp:
    ...
    def _load_server_config(self):
        ...
        try:
            ...
            config_path = get_resource_path(
                os.path.join("client", "server_config.json")
            )
    ...
    def process_queue(self):
        try:
            while True:
                ...
                elif message_type == "update_audit_log":
                    self.gui.update_audit_log(data)
    ...
    def start_scraping_on_server(self, server_address, params):
        if self.is_scraping:
            self.queue.put(
                ("log", "❌ Error: Ya hay un trabajo de scraping en progreso.")
            )
            return
        ...
        try:
            ...
            # Soporte para HTTPS y HTTP explícitos - Si ya viene con prefijo, usarlo directamente
            if ip_port.startswith("http://") or ip_port.startswith("https://"):
                self.server_base_url = ip_port
            elif ":" in ip_port and not ip_port.startswith("http"):
                # Formato host:puerto - inferir protocolo por el puerto
                host, port_str = ip_port.rsplit(":", 1)
                port = int(port_str) if port_str.isdigit() else 8001
                scheme = "https" if port == 443 else "http"
                self.server_base_url = f"{scheme}://{ip_port}"
            else:
        ...
        threading.Thread(
            target=self._start_scraping_thread, args=(params,), daemon=True
        ).start()

    def _start_scraping_thread(self, params):
        try:
            url = f"{self.server_base_url}/api/start_scraping"
    ...
    def stop_scraping(self):
        if not self.is_scraping or not self.server_base_url:
            self.queue.put(
                ("log", "No hay ninguna tarea de scraping activa para detener.")
            )
            return

        try:
            response = requests.post(
                f"{self.server_base_url}/api/stop_scraping", timeout=10
            )
            response.raise_for_status()
        ...
        while not self.stop_polling.is_set():
            try:
                response = requests.get(
                    f"{self.server_base_url}/api/detailed_status", timeout=10
                )
                response.raise_for_status()
                data = response.json()
                ...
                if self.is_scraping and self.scraping_start_time:
                    ...
                    if not is_job_running and elapsed_time > 10:
                        ...
                        logger.info(
                            "Todos los workers han finalizado. Deteniendo sondeo."
                        )
                        break
            ...
            try:
                events_url = (
                    f"{self.server_base_url}/api/events?min_id={self.last_event_id}"
                )
    ...
    def _send_captcha_response(self, captcha_id, solution):
        try:
            ...
            response.raise_for_status()
            self.queue.put(
                ("log", f"Solución de CAPTCHA {captcha_id} enviada al servidor.")
            )
        except requests.exceptions.RequestException as e:
    ...
    def force_reset_state(self):
        ...
        self.queue.put(
            (
                "log",
                "⚠️ Sistema reiniciado (Cliente + Servidor). El estado ahora es 'Inactivo'.",
            )
        )
