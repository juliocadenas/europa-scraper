...
class ClientApp:
        ...
        try:
            while True:
                ...
                if message_type == 'log':
                    ...
                    self.gui._render_status(data.get('workers', {}), data.get('courses', {}))
    ...
    def stop_scraping(self):
        ...
        try:
            response = requests.post(f"{self.server_base_url}/api/stop_scraping", timeout=10)
            response.raise_for_status()
    ...
    def _poll_status(self):
        ...
        while not self.stop_polling.is_set():
            try:
                response = requests.get(f"{self.server_base_url}/api/detailed_status", timeout=10)
                response.raise_for_status()
                data = response.json()
            ...
            try:
                events_url = f"{self.server_base_url}/api/events?min_id={self.last_event_id}"
