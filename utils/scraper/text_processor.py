...
URL_REF_PATTERN = re.compile(
    r"\s+(?:visita|ver|visitar|click|enlace|link|url|ver en|más en|más información en|leer más en)[:\s]+\S+$",
    re.IGNORECASE,
)
...
class TextProcessor:
    ...
    def estimate_keyword_occurrences(
        ...
        # Also count the FULL PHRASE as a whole
        search_term_lower = search_term.lower()
        if len(search_term) > 2:
            try:
                # Count exact phrase occurrences
                phrase_count = len(
                    re.findall(re.escape(search_term_lower), content_lower)
                )
                if phrase_count > 0:
                    word_counts[search_term.upper()] = phrase_count
            except re.error as e:
                logger.warning(f"Regex error for phrase '{search_term}': {e}")
    ...
    def should_exclude_result(
        ...
        """
        Determines if a result should be excluded based on keyword count and word limit.

        Args:
            total_words: Total number of words (for logging/legacy)
            word_counts: Dictionary with count of each keyword
            min_words: Minimum number of total words required on the page
            require_keywords: If True, requires at least 1 keyword occurrence to not be excluded
        ...
        if total_words < min_words:
            # We keep the string "total keywords" here because scraper_controller.py
            # explicitly checks for "total keywords" in the exclude reason
            return (
                True,
                f"Total keywords found... actually total words ({total_words}) less than minimum required ({min_words})",
            )

        # Check if there are no keyword matches (only if explicitly required)
        if require_keywords and total_keywords_found == 0:
            return (
                True,
                "Zero keywords found",
            )
